...
def upload_chunks_to_qdrant(documents):
    ...
    qdrant_client.upload_points(collection_name=QDRANT_COLLECTION_NAME, points=records_to_upload)
...
def reciprocal_rank_fusion(all_results: List[List[dict]], collection_name: str, k: int) -> List[str]:
    ...
    for doc_id, _ in top_k_docs:
        ...
        if records:
            departments.append(records[0].payload.get('research_paper_name'))
...
def getTopKDocs(query):
    return build_rrf_chain(query, "research_papers", 2)
